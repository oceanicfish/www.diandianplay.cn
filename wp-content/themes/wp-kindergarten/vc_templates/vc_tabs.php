<?php
global $smof_data;
$output = $title = $interval = $el_class = $cms_tab_bg_color = $cms_tab_icon = '';
 ...
 *
 */
$cms_tab_bg_color = !empty($atts['cms_tab_bg_color']) ? $atts['cms_tab_bg_color'] : ''; 
if ( isset( $matches[1] ) ) {
	$tab_titles = $matches[1];
}
$uqid = uniqid();
$class_link = 'cms-tab-'.$uqid;
$tabs_nav = '';
...
foreach ( $tab_titles as $tab ) {
    ...
    if(isset($tab_atts['title'])) {
        ...
        $tabs_nav .= '<li class="'.$class_link.'"><a style="background-color: '.$tab_atts['cms_tab_bg_color'].';border-color: '.$tab_atts['cms_tab_border_color'].';" href="#tab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) ) .'"><i class="fa '.$cms_tab_icon.'"></i>' . $tab_matches[1][0] . '<span style="background-color: '.$tab_atts['cms_tab_bg_color'].';border-color: '.$tab_atts['cms_tab_border_color'].';"></span></a></li>';
