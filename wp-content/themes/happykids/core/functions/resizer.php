...
if ( ! function_exists( 'bfi_wp_image_editor_check_notice' ) ) {
    function bfi_wp_image_editor_check_notice() {
        ...
        esc_html__( 'The server does not have ImageMagick or GD installed and/or enabled! Any of these libraries are required for WordPress to be able to resize images. Please contact your server administrator to enable this before continuing.', 'default' ));
    }
}
...
if ( ! class_exists( 'BFI_Image_Editor_GD_1_2' ) ) {
    ...
    class BFI_Image_Editor_GD_1_2 extends WP_Image_Editor_GD {
        ...
        public function rotate( $angle ) {
            ...
            return new WP_Error( 'image_rotate_error', esc_html__( 'Image rotate failed.', 'default' ), $this->file );
        }
        ...
        public function opacity( $opacity ) {
            ...
            return new WP_Error( 'image_opacity_error', esc_html__( 'Image opacity change failed.', 'default' ), $this->file );
        }
        ...
        public function colorize( $hexColor ) {
            ...
            return new WP_Error( 'image_colorize_error', esc_html__( 'Image color change failed.', 'default' ), $this->file );
        }
        ...
        public function grayscale() {
            ...
            return new WP_Error( 'image_grayscale_error', esc_html__( 'Image grayscale failed.', 'default' ), $this->file );
        }
        ...
        public function negate() {
            ...
            return new WP_Error( 'image_negate_error', esc_html__( 'Image negate failed.', 'default' ), $this->file );
        }
    }
}
...
if ( ! class_exists( 'BFI_Thumb_1_2' ) ) {
    ...
    class BFI_Thumb_1_2 {
        ...
        public static function thumb( $url, $params = array(), $single = true ) {
            ...
            $wp_content_part = substr( $upload_url, strlen( home_url() ) );
            $rel_path = '';
            $img_path = '';
            ...
            } else if ( strpos( $url, $wp_content_part ) !== false ) {
                $rel_path = $url;
                $img_path = $upload_dir . substr( $url, strpos( $url, $wp_content_part ) + strlen( $wp_content_part ) );
            }

            // Fail if we can't find the image in our WP local directory
            if ( empty( $img_path ) ) { return array(
            0 => $url,
            1 => NULL,
            2 => NULL,
            3 => array(
            ...
            if ( ! @file_exists( $img_path ) or ! getimagesize( $img_path ) ) { return array(
            0 => $url,
            1 => NULL,
            2 => NULL,
            3 => array(
            ...
            if ( isset( $width ) ) {
                if ( stripos( $width, '%' ) !== false ) {
                    ...
                    $retina_width = ( (int) ((float) str_replace( '%', '', $width ) * 2 ) / 100 * $orig_w);
                } else {
            ...
            if ( isset( $height ) ) {
                if ( stripos( $height, '%' ) !== false ) {
                    ...
                    $retina_height = ( (int) ((float) str_replace( '%', '', $height ) * 2 ) / 100 * $orig_h);
                } else {
            ...
            $retina_thumb = true;
            if ( ( isset( $retina_width ) && $retina_width > $orig_w ) || ( isset( $retina_height ) && $retina_height > $orig_h ) ) {
                $retina_thumb = false;
            }
            ...
            $destfilename_retina = "{$upload_dir}/{$dst_rel_path}-{$suffix}@2x.{$ext}";
            $img_url = "{$upload_url}/{$dst_rel_path}-{$suffix}.{$ext}";
            $retina_url = "{$upload_url}/{$dst_rel_path}-{$suffix}@2x.{$ext}";
            ...
            if ( @file_exists( $destfilename ) && @getimagesize( $destfilename ) ) {
                ...
                if ( $retina_thumb ) {
                    $retina_editor = wp_get_image_editor( $img_path );

                    if ( is_wp_error( $retina_editor ) ) { return false; }

                    if ( ( isset( $retina_width ) && $retina_width ) || ( isset( $retina_height ) && $retina_height ) ) {
                        if ( is_wp_error( $retina_editor->resize( isset( $retina_width ) ? $retina_width : null, isset( $retina_height ) ? $retina_height : null, isset( $crop ) ? $crop : false ) ) ) {
                            return false;
                        }
                    }
                    ...
                    if ( isset( $negate ) ) {
                        if ( $negate ) {
                            if ( is_wp_error( $retina_editor->negate() ) ) {
                                return false;
                            }
                        }
                    }
                    ...
                    if ( isset( $opacity ) ) {
                        if ( is_wp_error( $retina_editor->opacity( $opacity ) ) ) {
                            return false;
                        }
                    }
                    ...
                    if ( isset( $grayscale ) ) {
                        if ( $grayscale ) {
                            if ( is_wp_error( $retina_editor->grayscale() ) ) {
                                return false;
                            }
                        }
                    }
                    if ( isset( $color ) ) {
                        if ( is_wp_error( $retina_editor->colorize( $color ) ) ) {
                            return false;
                        }
                    }
                    $resized_retina = $retina_editor->save( $destfilename_retina, $mime_type );
                }
            }
