...
class TwitterOAuth
{
	...
	public function get($call, array $getParams = null)
	{
		...
		$this->method = 'GET';
		$this->resetParams();
		$this->cachefile = THEME_DIR . '/core/.tweetcache';
		...
		if (is_file($this->cachefile)) {
			if (version_compare(phpversion(), "5.3.0", ">=")) {
				...
				$cache = $this->_array_cast_recursive($cache);
			}
		}
	...
	private function _array_cast_recursive($array)	{
		if (is_array($array)) {
			foreach ($array as $key => $value) {
				if (is_array($value)) {
					$array[$key] = $this->_array_cast_recursive($value);
				}
				if ($value instanceof stdClass) {
					$array[$key] = $this->_array_cast_recursive((array)$value);
				}
			}
		}
		if ($array instanceof stdClass) {
			return $this->_array_cast_recursive((array)$array);
		}
		return $array;
	}
