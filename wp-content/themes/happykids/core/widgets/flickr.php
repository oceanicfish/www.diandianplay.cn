...
FLICKR;
		...
		function form( $instance ) {
			...
			$show = isset($instance['show']) ? $instance['show'] : 9;
			  ...
			  ?>
				<p><label><?php _e('Title: ', 'happykids'); ?><input name="<?php echo $this->get_field_name('title'); ?>"
			type="text" value="<?php echo $title; ?>" /></label></p>
			  <p><label><?php _e('Flickr Username:', 'happykids'); ?><br />@<input name="<?php echo $this->get_field_name('name'); ?>"
			type="text" value="<?php echo $name; ?>" /></label></p>
			  <p><label><?php _e('# of images to Show:', 'happykids'); ?></label>
